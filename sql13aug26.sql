...
drop database if exists windowsdb;
...
-- lag,lead ,ntile,row number [imp runnning sum,runnung count ]
select *,lead(amount,1) over() from orders;

select *,lead(amount,2) over(),lag(amount,1) over() from orders;
...
-- to add last 3 orders

select *,lag(amount,1,0) over(partition by customer_id order by order_date),
...
-- sum of 1 previous row and next one row
select customer_id,customer_name,amount,
sum(amount) over(partition by customer_id order by order_date 
rows between 1 preceding and 1 following) from orders;
