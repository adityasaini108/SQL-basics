...
-- Correlated Subquery  is a type of subquery also called as dependent query 
   -- In this, inner query depends on outer query
   -- In this, the inner query runs multiple times  for each row of outer query
...
select order_id,name,amount
 ...
 where amount=(select max(amount) from orders where name=o_outer.name);
...
salary=(select max(salary) from employee where dept_id=employee.dept_id);    -- XXXXXXXXXXXXX

select * from employee where dept_id=104
and salary>=all (select salary from employee where dept_id=104);

-- Employee whose salary is greater than the avg ssalary of their own department
...
-- Employees  who earn more than every other employees in their department
...
where salary >all (select salary from employee where dept_id=e.dept_id and emp_id 


-- employee who are not the highest paid employee in their department

select e.emp_name from employee as e
join department as d on e.dept_id=d.dept_id
where salary > (select Min(salary) from employee where d.dept_id=e.dept_id);

select emp_name from employee as e 
where salary != (select max(salary) from employee where dept_id=e.dept_id);

-- departments having at least one employee  earning more than 70000.
select dept_id,dept_name from department where dept_id =any (select dept_id from employee where salary > 70000);

-- find second highest paid  salary in each department.
...
where salary = (select max(salary) from employee as e2 where 
