...
-- find the top 2 orders per cutomer
select * from 
(select customer_id,customer_name,order_id,amount,rank() over(partition by customer_id order by amount desc) as customer_rank from orders) as t
...
-- percentage of customer total spending
